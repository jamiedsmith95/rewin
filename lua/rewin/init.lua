...
M.TeleMark = function(opts)
  ...
  local results = {}
  -- for line in marks.output:lines() do
  --   local str = string.find(line,'%w')
  ...
  for m in string.gmatch(marks.output, "([^\r\n]+)") do
    ...
    if str then
      if string.len(str) > 1 then
        -- print('too long',str)
      elseif string.match(str,"%w") and vim.api.nvim_get_mark(str,{}) then
        local buf = vim.api.nvim_get_mark(str,{})
        local ins = {}
        table.insert(ins,buf[1])
        -- table.insert(ins,buf[2])
        -- table.insert(ins,buf[3])
        table.insert(results,str)
      else
...
vim.keymap.set('n', '<leader>tm', function() M.TeleMark({ opts }) end)
return M
